...
namespace Sean.Utility.Barcode
{
    ...
    public class Code39
    {
        private readonly Dictionary<char, string> _validChar = new Dictionary<char, string>()
        {
            {'0',"000110100"},
            {'1',"100100001"},
            {'2',"001100001"},
        ...
        /// <param name="showContent">是否显示条码内容</param>
        /// <returns>Code39条形码</returns>
        public Bitmap GetImage(string content, int height, bool showContent)
        {
            int topMargin = 0;
            int leftMargin = 5;
            int thickLength = 2;
            ...
            if (showContent)
            {
                ...
                graphics.DrawString(content, font, Brushes.Black, x, y);
            }
